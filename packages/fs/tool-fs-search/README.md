...
- **System `rg` must be on PATH** — resolution probes `PATH` for an executable `rg` (Windows: `rg.exe`) at the first call; a missing binary fails calls with `SEARCH_FAILED` and the probe detail names the searched entries (HiShell provides `rg` via harmonybrew). Remote or virtual filesystems need a co-located workspace or another search consumer.
